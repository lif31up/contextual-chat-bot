...
class Loader(Dataset):
    def __init__(self, filename):
        ...
        with open(filename) as y_file: self.data = yaml.load(y_file, Loader=yaml.FullLoader)
        xy = list()
        for intent in self.data:
            tag = intent["tag"]
            self.tags.append(tag)
            for pattern in intent["patterns"]:
                tokenized_pattern = self.transformer.tokenize(pattern)
        ...
        self.transformer.wordbook = self.dictionary

        patterns, targets = list(), list()
        for x, y in xy:
            bag = self.transformer.bag(x)
            patterns.append(bag)
    ...
    def __getitem__(self, index): return self.patterns[index], self.targets[index]
    def __len__(self): return self.n_patterns
# yaml_loader
